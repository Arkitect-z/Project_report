...
class TokenHMR(pl.LightningModule):
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        # 读取hmr4d_results.pt中的body_pose数据
        hmr4d_results = torch.load("hmr4d_results.pt")
        original_pose = pred_smpl_params["body_pose"]
        # 获取body_pose数据
        body_pose_matrix = hmr4d_results  # (max_length, num_sequences, 23, 3, 3)
        # 将body_pose_matrix赋值给pred_smpl_params中的body_pose
        pred_smpl_params["body_pose"] = body_pose_matrix[batch['num_x']][0:batch_size]
        alpha = 0.3  # 调节因子
        pred_smpl_params["body_pose"] = alpha * original_pose + (1-alpha) * pred_smpl_params["body_pose"]

        # print(pred_smpl_params["body_pose"].shape)
